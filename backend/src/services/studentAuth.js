...
/** Google / Firebase ID token → student JWT session */
export async function loginStudentWithGoogle(idToken) {
  const profile = await verifyGoogleIdToken(idToken);
  if (!profile?.email) {
    throw Object.assign(new Error('Invalid Google token'), { status: 401 });
  }
  let student = await store.getStudentByEmail(profile.email);
  if (!student) {
    const id = `STU_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
    student = {
      id,
    ...
    // Link Google provider on existing email account
    student.auth_provider = student.auth_provider === 'password' ? 'password+google' : student.auth_provider || 'google';
    if (profile.sub) student.googleSub = profile.sub;
    if (profile.picture) student.picture = profile.picture;
    if (profile.name && !student.name) student.name = profile.name;
    await store.saveStudent(student);
  }
  return {
    token: signStudent(student),
