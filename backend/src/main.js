...
import { registerStudent, loginStudent, studentFromHeaders } from './services/studentAuth.js';
import { effectiveExamStatus, withEffectiveStatus, calculateAttemptScore, secondsLeft, rankOfficialAttempts } from './services/scoring.js';
import { parseQuestionsFromMedia } from './services/ocr.js';
import { assertProductionSecrets, corsHeaders, rateLimit, getJwtSecret } from './middleware/security.js';
import { ownsExam, ownsQuestion, ownsStudent, ownsAttempt, teacherIdOf, notFoundOrForbidden } from './middleware/ownership.js';
...
export default async ({ req, res, log, error }) => {
  ...
  try {
    ...
    if (path.startsWith('/api/telegram') || path.startsWith('/api/webapp')) {
      return json(res, 410, { error: 'Telegram APIs removed. Students use /api/student/* with email login.' }, req);
    }

    // Student auth
    if (path === '/api/student/auth/register' && method === 'POST') {
      try { return json(res, 201, await registerStudent(body), req); }
      catch (e) { return json(res, e.status || 400, { error: e.message }, req); }
    }
    if (path === '/api/student/auth/login' && method === 'POST') {
      try { return json(res, 200, await loginStudent(body), req); }
      catch (e) { return json(res, e.status || 401, { error: e.message }, req); }
    }
    if (path === '/api/student/auth/me' && method === 'GET') {
      ...
      return json(res, 200, { student: { id: s.id, email: s.email, name: full?.name || s.name } }, req);
    }
    if (path === '/api/student/exams' && method === 'GET') {
      const s = studentFromHeaders(req.headers || {});
      if (!s) return json(res, 401, { error: 'UNAUTHORIZED' }, req);
      const exams = (await store.getExams()).filter((e) => e.status !== 'DRAFT' && e.status !== 'CANCELLED');
    ...
    if (path === '/api/student/start' && method === 'POST') {
      ...
      if (!exam.questions?.length) {
        const qs = await store.getQuestions(exam.id);
        exam.questions = qs;
      }
      const questions = (exam.questions || []).map((q) => ({
        id: q.id, question: q.question || q.text || '', options: q.options || [],
        marks: q.marks ?? 1, imageFileId: q.image?.fileId || q.imageFileId || null,
      }));
      ...
      if (existing && !body.forceNew) {
        return json(res, 200, { attempt: existing, exam, questions, secondsLeft: 3600 }, req);
      }
      ...
      const attempt = {
        id: ID.unique(), examId: exam.id, studentId: student.id, studentName: student.name,
        studentEmail: student.email, status: 'IN_PROGRESS', answers: {}, currentQuestionIndex: 0,
        startedAt, endsAt, expiresAt: endsAt, durationMinutes,
        attemptNumber: await store.nextAttemptNumber(exam.id, student.id),
        practice: !!body.practice, isOfficial: !body.practice, pausedAt: null, pausedSeconds: 0,
      };
      await store.saveAttempt(attempt);
      return json(res, 200, { attempt, exam: { id: exam.id, title: exam.title, durationMinutes }, questions, secondsLeft: durationMinutes * 60 }, req);
    }
    if (path === '/api/student/sync' && method === 'POST') {
      ...
      if (attempt.status !== 'IN_PROGRESS') return json(res, 200, { attempt }, req);
    ...
    if (path === '/api/student/submit' && method === 'POST') {
      ...
      if (!attempt) return json(res, 404, { error: 'Attempt not found' }, req);
      if (attempt.status === 'SUBMITTED' || attempt.status === 'AUTO_SUBMITTED') return json(res, 200, { attempt }, req);
      ...
      const stats = calculateAttemptScore(exam, attempt.answers || {}, 0);
      Object.assign(attempt, stats, { status: 'SUBMITTED', submittedAt: new Date().toISOString() });
      await store.saveAttempt(attempt);
    ...
    if (path === '/api/student/results' && method === 'GET') {
      const s = studentFromHeaders(req.headers || {});
      if (!s) return json(res, 401, { error: 'UNAUTHORIZED' }, req);
      const attempts = (await store.getAttempts()).filter((a) => a.studentId === s.id && (a.status === 'SUBMITTED' || a.status === 'AUTO_SUBMITTED'));
