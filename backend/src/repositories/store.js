...
export const store = {
  ...
  async getStudentByEmail(email) {
    ...
    return all.find((s) => String(s.email || '').toLowerCase() === e) || null;
  },
  ...
  async getStudentAttempts(examId, studentKey) {
    const all = await listEntity('attempt', 3000);
    const key = String(studentKey ?? '');
    return all.filter((a) => {
      if (a.examId !== examId) return false;
      if (a.studentId && String(a.studentId) === key) return true;
      if (a.telegramUserId != null && String(a.telegramUserId) === key) return true;
      return false;
    });
  },
  ...
  async getSettings() {
    ...
    return { ...defaults, ...(s || {}), botActive: false };
  },
